...
class SJF
 {
	...
	static void arrangeArrival(int num, int[][] mat) {
		for (int i = 0; i < num; i++) {
			for (int j = 0; j < num - i - 1; j++) {
				if (mat[j][1] > mat[j + 1][1] || (mat[j][1] == mat[j+1][1] && mat[j][2] > mat[j+1][2])) {
					for (int k = 0; k < 5; k++) {
						int temp = mat[j][k];
						mat[j][k] = mat[j + 1][k];
						mat[j + 1][k] = temp;
	...
	public static void main(String[] args) {
        ...
        for(int i=0;i<num;i++)
        sum+=mat[i][5];
        System.out.println("Average Turn-atound Time(TAT): "+sum/num);

        sum=0;
        for(int i=0;i<num;i++)
        sum+=mat[i][4];
        System.out.println("Average Waiting Time(WT): "+sum/num);
